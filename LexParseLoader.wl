...
(* ::Text::GrayLevel[0.5]:: *)
(* Autogenerated LexParse loader file *)


Temp`PackageScope`LexParseLoading`Private`$DependencyLoad=
  TrueQ@Temp`PackageScope`LexParseLoading`Private`$DependencyLoad;
If[Temp`PackageScope`LexParseLoading`Private`$DependencyLoad,
  Unprotect["`LexParse`PackageScope`Private`Constants`$TopLevelLoad"];
  Evaluate[Symbol["`LexParse`PackageScope`Private`Constants`$TopLevelLoad"]]=False,
  Unprotect["LexParse`PackageScope`Private`Constants`$TopLevelLoad"];
  Evaluate[Symbol["LexParse`PackageScope`Private`Constants`$TopLevelLoad"]]=
    MemberQ[$ContextPath, "Global`"]
  ];


If[Temp`PackageScope`LexParseLoading`Private`$DependencyLoad,
  Evaluate[Symbol["`LexParse`PackageScope`Private`$RootContext"]]=$Context;
  BeginPackage["`LexParse`"],
  Evaluate[Symbol["LexParse`PackageScope`Private`$RootContext"]]="";
  BeginPackage["LexParse`"]
  ]


ClearAll[LexParse];
LexParse::usage="LexParse is a manager head for the LexParse package";
...
LexParse["LoadingParameters"]:=$PackageLoadSpecs
$PackageLoadSpecs=
  Merge[
    {
      With[
...
LexParse["Mode"]:=$PackageLoadingMode
$PackageLoadingMode=
...
LexParse["Dependencies"]:=$PackageDependencies;
$PackageDependencies=
...
LexParse["DependencyBase"]:=$PackageDependencyBase;
...
LexParse["PackageListing"]:=$PackageListing;
$PackageListing=<||>;
LexParse["Contexts"]:=$PackageContexts;
  ...
  $PackageContexts=
    Join[
      If[$PackageLoadingMode==="Dependency",
        ...
        {
          "LexParse`",
          "LexParse`PackageScope`Private`",
          "LexParse`PackageScope`Package`"
          },
...
LexParse["FEScopedSymbols"]:=$PackageFEHiddenSymbols;
$PackageFEHiddenSymbols={};
LexParse["PackageScopedSymbols"]:=$PackageScopedSymbols;
...
LexParse["FileContexts"]:=$PackageFileContexts;
...
LexParse["DeclaredPackages"]:=$DeclaredPackages;
...
LexParse["LoadedPackages"]:=$LoadedPackages;
...
PackageFileContext[f_String?DirectoryQ]:=
  With[{s=PackageFileContextPath[f]},
    StringRiffle[Append[""]@Prepend[s,LexParse],"`"]
    ];
...
LexParse::nodep="Couldn't load dependency `` of type ``";
LexParse::nodup="Couldn't update dependency `` of type ``";
  ...
  Block[{retcode, site, path, file, tmp},
    ...
    site=
      ...
      If[retcode<300,
        ...
        DeleteFile[tmp];
        file,
        Message[LexParse::nodep, dep, "Package"];
        DeleteFile[tmp];
        $Failed
        ]
      ];
    ...
    Monitor[
      MapThread[
        Check[
          ...
          Message[LexParse::nodep, #2, "Paclet"];
          $Failed
          ]&,
        {
          pacs,
      ...
      Monitor[
        MapThread[
          Check[
            ...
            Message[LexParse::nodup, #2, "Paclet"];
            $Failed
            ]&,
          {
            StringDelete[deps, "`"],
...
$PackageErrorMessage=
  "LexParse encountered exception ``";
...
PackageCatchMessage[
  ...
  Replace[
    Thread[Map[Hold[MessageName[LexParse, #]]&, msg], Hold],
    {
      {}:>
        Check[expr, 
...
PackageFailureException[
  ...
  (
    Set[MessageName[LexParse, tag], body];
    PackageFailureException[
      MessageName[LexParse, tag],
      body,
      ops
...
PackageAttachMainAutocomplete[]:=
  PackageAddAutocompletions[LexParse, 
    Table[
      Replace[{}->None]@
        Cases[
          DownValues[LexParse],
...
PackagePrepPackageSymbol[]:=
  Switch[$AllowPackageSymbolDefinitions,
    None,
      If[Length@OwnValues[LexParse]==0,
        Remove[LexParse],
        DownValues[LexParse]={}
        ],
    False,
...
If[Temp`PackageScope`LexParseLoading`Private`$DependencyLoad,
  If[
    (Clear@`LexParse`PackageScope`Private`$loadAbort;!#)&@
      `LexParse`PackageScope`Private`$loadAbort,
    Unprotect[`LexParse`PackageScope`Private`$LoadCompleted];
    `LexParse`PackageScope`Private`$LoadCompleted=True;
    `LexParse`PackageScope`Private`PackagePostProcessContextPathReassign[]
    ],
  If[
    (Clear@LexParse`PackageScope`Private`$loadAbort;!#)&@
      LexParse`PackageScope`Private`$loadAbort,
...
Remove["Temp`PackageScope`LexParseLoading`Private`*"]